  ...
  <div class="commission">
    ...
    <Section>
      ...
      <component
        ...
        :to="{ name: 'council-slug', params: { slug: mainMember.slug } }"
      >
      ...
      <div v-if="members.length" class="commission-members">
        <PersonCard
          ...
          :to="{ name: 'council-slug', params: { slug: member.slug } }"
...
export default {
  ...
  async asyncData({ $api }) {
    const apiMethod = $api.pages.council

    if (!apiMethod) return { page: {} }

    const page = await apiMethod().then(({ data }) => data)
    const { data: members } = await $api.members.getList('soviet')
